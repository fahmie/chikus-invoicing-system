{{-- @extends('layouts.app', ['page' => 'inventory_managment']) --}}
        ...
        <div class="flex">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">{{__('Deduct Stock')}}</li>
                </ol>
            </nav>
            <h1 class="m-0">{{__('Deduct Stock')}}</h1>
        </div>
    </div>
@endsection
 
@section('content') 
   <form action="{{ route('managements.stockout_store') }}" method="POST" enctype="multipart/form-data">
        @include('layouts._form_errors')
        @csrf
        
        @include('application.inventory_management._form_deduct')
    </form>
@endsection
...
<script>
  ...
  $("#materialChecked1").click(function() {
      // var checkedValue = $('#customCheckbox2').val();
      var x = document.getElementById('main_item');
  ...
  $(document).ready(function() {
        $('#supplier_id').on('change', function() {
            ...
            if (stateID) {
                $.ajax({
                    url: '/getProductBySupplierID/' + stateID, 
                    type: "GET",
                    data: {
                        "_token": "{{ csrf_token() }}"
                    },
                    ...
                    success: function(data) {
                        ...
                        if (data) {
                            $('#product_id').empty();
                            $('#product_id').focus;
                            $('#product_id').append('<option disabled selected value="">Select product</option>');

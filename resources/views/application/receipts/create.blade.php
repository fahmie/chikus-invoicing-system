        ...
        <div class="flex">
            <nav aria-label="breadcrumb">
                <ol class="breadcrumb mb-0">
                    ...
                    <li class="breadcrumb-item active" aria-current="page">Create Receipts</li>
                </ol>
            </nav>
            <h1 class="m-0">Create Receipts</h1>
        </div>
    </div>
@endsection
...
<div class="card">
        ...
        @include('application.receipts._filters4')
        @include('application.receipts._form_report', ['action' => route('receipts.report')])
        <div class="card card-form" style="background-color: #F3F5F6;box-shadow: none;">
                <form action="{{ route('receipts.store') }}" method="POST" id="receipt">
                        @include('layouts._form_errors')
                        @csrf

                        
                        @include('application.receipts._form')
                </form>
            </div>
        </div>
    </div>

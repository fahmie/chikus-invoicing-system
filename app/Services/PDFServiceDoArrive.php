...
class PDFServiceDoArrive extends PDF_ImageAlpha
{
    var $font = 'DejaVu';
    ...
    function setReference2($reference1)
    {
        $this->reference1 = $reference1;
    }
    function setReference3($reference2)
    {
    ...
    function addParagraph1($paragraph1)
    {
        if ($paragraph1 != '') {

            $paragraph1 = $this->br2nl($paragraph1);
            $this->addText[] = array('paragraph1', $paragraph1);
            $this->extraNotes = true;
        }
    }
    function addParagraph2($paragraph2)
    {
        if ($paragraph2 != '') {

            $paragraph2 = $this->br2nl($paragraph2);
            $this->addText[] = array('paragraph2', $paragraph2);
            $this->extraNotes = true;
        }
    }
    ...
    function Header()
    {
        ...
        if ($this->PageNo() == 1) {
            ...
            $positionX = $this->document['w'] - $this->margins['l'] - $this->margins['r'] - max(strtoupper($this->GetStringWidth($this->l['number'])), strtoupper($this->GetStringWidth($this->l['date'])), strtoupper($this->GetStringWidth($this->l['due']))) - 50;
            ...
            if ($this->reference) {
                ...
                $this->SetFont($this->font, 'B', 9);
                $this->SetTextColor($this->color[0], $this->color[1], $this->color[2]);
                $this->Cell(32, $lineheight, strtoupper($this->l['number1']) . ':', 0, 0, 'L');
                $this->SetTextColor(50, 50, 50);
                $this->SetFont($this->font, '', 9);
            ...
            if(!$this->hide_header) {
                ...
                if (!isset($this->productsEnded)) {
                    ...
                    $this->Cell($this->columnSpacing, 10, '', 0, 0, 'L', 0);
                    $this->Cell($width_other, 10, strtoupper($this->l['unit']), 0, 0, 'C', 1);
                    $this->Ln();
                } else {
                    $this->Ln(12);
                }
    ...
    function Body()
    {
        ...
        if ($this->items) {
            foreach ($this->items as $item) {
                if ($item['description']) {
                    //Precalculate height
                    $calculateHeight = new PDFServiceDoArrive;
                ...
                $x = $this->GetX();
                $this->Cell(119.8, $cHeight, $item['item'], 0, 0, 'L', 1);
                if ($item['description']) {
                    $resetX = $this->GetX();
                    $resetY = $this->GetY();
        ...
        if ($this->totals) {
            foreach ($this->totals as $total) {
                ...
                $this->Cell($width_other - 1, $cellHeight, $total['name'], 0, 0, 'C', 1);
                $this->Cell($this->columnSpacing, $cellHeight, '', 0, 0, 'L', 0);
                $this->SetFont($this->font, 'b', 8);
                $this->SetFillColor($bgcolor, $bgcolor, $bgcolor);
                if ($total['colored']) {
        ...
        if ($this->extraNotes == true) { 
            foreach ($this->addText as $text) {
                if ($text[0] == 'title') {
                    ...
                    $this->Cell(0, 6, strtoupper($text[2]), 0, 0, 'L', 1);
                    $this->Ln();
                }
                if ($text[0] == 'paragraph2') {
                    $this->Ln(5);
                    $this->SetTextColor(80, 80, 80);
                    $this->SetFont($this->font, '', 8);
                    $this->MultiCell(0, 40, $text[1], 0, 'L', 0);
                    $this->Ln(5);
                }
                if ($text[0] == 'paragraph') {
                    $this->Ln(5);
                    ...
                    $this->MultiCell(0, 15, $text[1], 0, 'L', 0);
                    $this->Ln(5);
                }
                if ($text[0] == 'paragraph1') {
                    $this->Ln(5);
                    $this->SetTextColor(80, 80, 80);
                    $this->SetFont($this->font, 'b', 10);
                    $this->MultiCell(0, 5, $text[1], 0, 'L', 0);
                    $this->Ln(5);
                }
            }
        }
    ...
    private function setLanguage($language)
    {
        ...
        $l = [];
        $l['number'] = __('messages.do_id');
        $l['number1'] = __('messages.invoice_id');
        ...
        $l['Qtyarrive'] = __('Quantity Arrived');
        $l['vat'] = __('messages.tax');
        $l['total'] = __('messages.total');
        $l['unit'] = __('Total Unit');

...
class PDFServiceRec extends PDF_ImageAlpha
{
    var $font = 'DejaVu';
    ...
    function Header()
    {
        ...
        if ($this->PageNo() == 1) {
            ...
            $positionX = $this->document['w'] - $this->margins['l'] - $this->margins['r'] - max(strtoupper($this->GetStringWidth($this->l['rec'])), strtoupper($this->GetStringWidth($this->l['date'])), strtoupper($this->GetStringWidth($this->l['due']))) - 60;
    ...
    function Body()
    {
        ...
        if ($this->items) {
            foreach ($this->items as $item) {
                if ($item['description']) {
                    //Precalculate height
                    $calculateHeight = new PDFServiceRec;

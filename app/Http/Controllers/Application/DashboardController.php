...
class DashboardController extends Controller
{
    ...
    public function index(Request $request)
    {
       ...
       // return response()->json($site);
        ...
        if(Auth::user()->roles =="superadmin"){
            ...
            $otws = Invoice::where('status', 'OTW')->latest()->paginate(10); 
            $pettyCash = PettyCash::latest()->paginate(10);
            ...
            $otws = Invoice::where('status', 'OTW')->whereIn('sites_id', $sites_id)->latest()->paginate(10);
            $pettyCash = PettyCash::whereIn('sites_id', $sites_id)->latest()->paginate(10);
            $month_year = DB::table('invoices')
            ->select(DB::raw('YEAR(created_at) as year'))
            ...
            $otws = Invoice::where('status', 'OTW')->where('sites_id', $currentSites)->latest()->paginate(10);
            $pettyCash = PettyCash::where('sites_id', $currentSites)->latest()->paginate(10);
            $month_year = DB::table('invoices')
            ->select(DB::raw('YEAR(created_at) as year'))
        ...
        $completed = Invoice::where('status', 'COMPLETED')->latest()->paginate(5);
        // $month_year = DB::table('invoices')
        // ->select(DB::raw('YEAR(created_at) as year'))
        // ->distinct()
        // ->get();
    ...
    public function report(Request $request)
    {
     ...
     set_time_limit(300000); // Extends to 5 minutes.
       $pdf = PDF::loadView('application.dashboard.report', ['data' => $data, 'client' => $client, 'product' => $product,'total_mt' => $total_mt, 
      'total_price' => $total_price,'currentCompany' => $currentCompany,'companys' => $companys, 'total_sales_rm' => $total_sales_rm, 'total_sales_contract_rm' => $total_sales_contract_rm,
      'total_sales_cash_rm' => $total_sales_cash_rm,'total_sales_mt' => $total_sales_mt, 'total_sales_contract_mt' => $total_sales_contract_mt, 'total_sales_cash_mt' => $total_sales_cash_mt]);
